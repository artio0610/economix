	...
	var methods = {
		init: function (options) {
			var p = {
				...
				end: function(){},						//Triggered when the tour came to the end, or was interrupted.
					...
					var overlayPos = function(targetEl){
						...
						if(targetEl && targetEl.length){
							...
							if(hData.opt.steps){
								...
								hData.v.hContPos.stop(true).delay(500).fadeIn(200);
							}else{
										...
										if(itour_data.tourId == hData.v.tourid){
											if(hWrap.data().itour_go){
												...
												if(hData.opt.tourContinue){
													...
													if(nowLoc.search(pageLoc) === -1 && pageLoc.search(nowLoc) === -1 && nowLoc !== pageLoc){
											...
											if(nowLoc.search(targetLoc) === -1 && targetLoc.search(nowLoc) === -1 && nowLoc !== targetLoc){
